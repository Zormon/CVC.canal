...
class Content {
    ...
    next() {
        ...
        if(!this.music.isFading && this.contenidos != null && !this.paused) {
            ...
            if (this.contenidos.length > 0) { // Hay contenidos
                ...
                this.ipc.send('log', {origin: 'MEDIA', event: 'NEXT', message: `Nombre: ${this.current.nombre}, Volumen: ${this.current.volumen}`})
    ...
    togglePause() {
        if (!this.paused) { // Pause
            ...
            this.ipc.send('log', {origin: 'MEDIA', event: 'RESUME', message: `Nombre: ${this.current.nombre}, Volumen: ${this.current.volumen}`})
        }
